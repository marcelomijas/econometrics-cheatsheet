...
% page foot
\cfoot{\href{https://github.com/marcelomijas/econometrics-cheatsheet}{\normalfont \footnotesize Versión 2.7-es - github.com/marcelomijas/econometrics-cheatsheet - CC BY 4.0}}
...
\section*{Contrastes de hipótesis}
	...
	\subsection*{Contraste F}
		...
		\begin{itemize}[leftmargin=*]
			\item \textbf{$\sum_{i=1}^n \hat{u}_{N}^2$} - es la Suma de Resid. Cuadrados del modelo no restringido ($SRC_{N}$).
			\item \textbf{$\sum_{i=1}^n \hat{u}_R^2$} - es la Suma de Resid. Cuadrados del modelo restringido ($SRC_R$).
		\end{itemize}
		Bajo $H_0$:
		\begin{center}
			$F = \frac{SRC_R - SRC_{N}}{SRC_{N}} \frac{(n-k_{N}-1)}{q} \sim F_{q, n-k_{N}-1}$
		\end{center}
		Donde $k_{N}$ es el número de parámetros del modelo no restringido y $q$ es el número de hipótesis lineales a probar.
		\\ Si $F_{q, n-k_{N}-1} < F$, existe evidencia para rechazar $H_0$.
