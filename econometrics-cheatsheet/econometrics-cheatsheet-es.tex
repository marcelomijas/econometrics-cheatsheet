...
% page foot
\cfoot{\href{https://github.com/marcelomijas/econometrics-cheatsheet}{\normalfont \footnotesize 2.9.1-es - github.com/marcelomijas/econometrics-cheatsheet - CC BY 4.0}}
...
\section*{Supuestos y propiedades}
	...
	\subsection*{Propiedades asintóticas de MCO}
		...
		\begin{itemize}[leftmargin=*]
			...
			\item De (1) a (4): MCO es \textbf{consistente}. $\mathrm{p.lim}(\hat{\beta}_j) = \beta_j$ (a (4b) sin (4a), exogeneidad débil, insesg. y consistente).
...
\section*{Mínimos Cuadrados Ordinarios}
	...
	\subsection*{Modelo de regresión múltiple}
		...
		% start text
			...
			\begin{center}
				...
				$\hat{\beta}_j = \frac{\Cov(y, \text{residualizado } x_j)}{\Var(\text{residualizado } x_j)}$
			\end{center}
			\vspace*{1mm}
	...
	\subsection*{Medidas de error}
		...
		Error Estándar de la Regresión: \hfill $\hat{\sigma} = \sqrt{\frac{\SRC}{n-k-1}}$ \\
		Error Estándar de los $\hat{\beta}$'s: \hfill $\ee(\hat{\beta}) = \hat{\sigma} \cdot \sqrt{(X^T X)^{-1}}$ \\
		Error Cuadrático Medio: \hfill $\ECM = \frac{1}{n} \sum_{i=1}^n (y_i - \hat{y}_i)^2$ \\
		Error Medio Absoluto: \hfill $\EMA = \frac{\sum_{i=1}^n |y_i - \hat{y}_i|}{n}$ \\
		Porcentaje Medio de Error: \hfill $\PME = \frac{\sum_{i=1}^n |\hat{u}_i / y_i|}{n} \cdot 100$
...
\section*{Contrastes de hipótesis}
	...
	\subsection*{Contraste F}
		...
		\begin{itemize}[leftmargin=*]
			\item \textbf{$\sum_{i=1}^n \hat{u}_{\mathrm{nr}}^2$} - es la Suma de Resid. Cuadrados del modelo no restringido ($\SRC_{\mathrm{nr}}$).
			\item \textbf{$\sum_{i=1}^n \hat{u}_\mathrm{r}^2$} - es la Suma de Resid. Cuadrados del modelo restringido ($\SRC_\mathrm{r}$).
		\end{itemize}
		Bajo $H_0$:
		\begin{center}
			$F = \frac{\SRC_\mathrm{r} - \SRC_{\mathrm{nr}}}{\SRC_{\mathrm{nr}}} \frac{(n-k_{\mathrm{nr}}-1)}{q} \sim F_{q, n-k_{\mathrm{nr}}-1}$
		\end{center}
		Donde $k_{\mathrm{nr}}$ es el número de parámetros del modelo no restringido y $q$ es el número de hipótesis lineales a probar. \\
		Si $F_{q, n-k_{\mathrm{nr}}-1} < F$, existe evidencia para rechazar $H_0$. \\
...
\section*{Heterocedasticidad}
	...
	\subsection*{Corrección}
		\begin{itemize}[leftmargin=*]
			...
			\begin{itemize}[leftmargin=*]
				\item Suponiendo que $\Var(u_i) = \sigma^2 \cdot x_i$, dividir las variables del modelo entre la raíz cuadrada de $x_i$ y aplicar MCO.
				\item Suponiendo que $\Var(u_i) = \sigma^2 \cdot x_i^2$, dividir las variables del modelo entre $x_i$ (la raíz cuadrada de $x_i^2$) y aplicar MCO.
			\end{itemize}
