...
% data points
\draw plot [only marks, mark=*, mark size=5, domain=-8:8, samples=20] (\x,{rnd * 5 - 1.5 + 0.5 * \x});
% beta0
...
% data points
\draw plot [only marks, mark=*, mark size=5, domain=-8:8, samples=20] (\x,{rnd * 6.5 - 1.5 + 0.5 * \x});
% beta0
\draw (-9.3,-9.6) -- (-9.5,-9.6) -- (-9.5,-4.3) -- (-9.3, -4.3) node[anchor=north west] {$\beta_0$};
\end{tikzpicture}
...
Where: \\ $\hat{\beta}_0 = \overline{y} - \hat{\beta}_1 \overline{x}_1 - ... - \hat{\beta}_k \overline{x}_k$ \\ $\hat{\beta}_j = \frac{Cov(y, resid(x_j))}{Var(resid(x_j))}$

\vspace*{1mm}

In matrix form: \\ $\hat{\beta} = (X^T X)^{-1} X^T Y$
