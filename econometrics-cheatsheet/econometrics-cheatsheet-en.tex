...
% page foot
\cfoot{\href{https://github.com/marcelomijas/econometrics-cheatsheet}{\normalfont \footnotesize 2.9.1-en - github.com/marcelomijas/econometrics-cheatsheet - CC BY 4.0}}
...
\columnbreak
% start of column 2 and 3 page 1
\section*{Assumptions and properties}
	...
	\subsection*{Asymptotic properties of OLS}
		...
		\begin{itemize}[leftmargin=*]
			...
			\item Hold (1) to (4): OLS is \textbf{consistent}. $\mathrm{p.lim}(\hat{\beta}_j) = \beta_j$ (to (4b) left out (4a), weak exogeneity, biased but consistent)
...
\section*{Ordinary Least Squares}
	...
	\subsection*{Multiple regression model}
		...
		% start text
			...
			\begin{center}
				...
				$\hat{\beta}_j = \frac{\Cov(y, \text{residualized } x_j)}{\Var(\text{residualized } x_j)}$
			\end{center}
			\vspace*{1mm}
	...
	\subsection*{Interpretation of coefficients}
		\begin{center}
			\scalebox{0.86}{ % scale the table to fit in the column
	...
	\subsection*{Error measures}
		...
		Standard Error of the $\hat{\beta}$'s: \hfill $\se(\hat{\beta}) = \hat{\sigma} \cdot \sqrt{(X^T X)^{-1}}$ \\
		Mean Squared Error: \hfill $\MSE = \frac{1}{n} \sum_{i=1}^n (y_i - \hat{y}_i)^2$ \\
		Absolute Mean Error: \hfill $\AME = \frac{\sum_{i=1}^n |y_i - \hat{y}_i|}{n}$ \\
		Mean Percentage Error: \hfill $\MPE = \frac{\sum_{i=1}^n |\hat{u}_i / y_i|}{n} \cdot 100$
% end of column 2 and 3 page 1
\columnbreak
% start of column 1, 2 and 3 page 2
\section*{R-squared}
...
\section*{Hypothesis testing}
	\subsection*{The basics of hypothesis testing}
		...
		\begin{itemize}[leftmargin=*]
			...
			\item \textbf{Statistic of contrast} - is a random variable whose probability distribution is known under the null hypothesis.
			\item \textbf{Critic value} - is the value against which the statistic of contrast is compared to determine if the null hypothesis is rejected or not. Is the value that makes the frontier between the regions of acceptance and rejection of the null hypothesis.
	...
	\subsection*{The F contrast}
		...
		\begin{itemize}[leftmargin=*]
			\item \textbf{$\sum_{i=1}^n \hat{u}_{\mathrm{nr}}^2$} - is the Sum of Sq. Resid. of the non restricted model ($\SSR_{\mathrm{nr}}$).
			\item \textbf{$\sum_{i=1}^n \hat{u}_\mathrm{r}^2$} - is the Sum of Sq. Resid of the restricted model ($\SSR_\mathrm{r}$).
		\end{itemize}
		Under $H_0$:
		\begin{center}
			$F = \frac{\SSR_\mathrm{r} - \SSR_{\mathrm{nr}}}{\SSR_{\mathrm{nr}}} \frac{(n-k_{\mathrm{nr}}-1)}{q} \sim F_{q, n-k_{\mathrm{nr}}-1}$
		\end{center}
		Where $k_{\mathrm{nr}}$ is the number of parameters of the non restricted model and $q$ is the number of linear hypothesis tested. \\
		If $F_{q, n-k_{\mathrm{nr}}-1} < F$, there is evidence to reject $H_0$. \\
...
% end of column 1, 2 and 3 page 2
\columnbreak
...
\section*{Heteroscedasticity}
	...
	\subsection*{Correction}
		\begin{itemize}[leftmargin=*]
			...
			\begin{itemize}[leftmargin=*]
				\item Supposing that $\Var(u_i) = \sigma^2 \cdot x_i$, divide the model variables by the square root of $x_i$ and apply OLS.
				\item Supposing that $\Var(u_i) = \sigma^2 \cdot x_i^2$, divide the model variables by $x_i$ (the square root of $x_i^2$) and apply OLS.
			\end{itemize}
