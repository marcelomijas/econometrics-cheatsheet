...
\cfoot{\href{https://github.com/marcelomijas/econometrics-cheatsheet}{\normalfont \footnotesize CS-24.5-EN - github.com/marcelomijas/econometrics-cheatsheet - CC-BY-4.0 license}}
...
\begin{itemize}[leftmargin=*]
	...
	\item \textbf{Alternative hypothesis} ($H_1$) - is the hypothesis that cannot be rejected when $H_0$ is rejected.
	\item \textbf{Test statistic} - is a random variable whose probability distribution is known under $H_0$.
	\item \textbf{Critical value} ($C$) - is the value against which the test statistic is compared to determine if $H_0$ is rejected or not. It sets the frontier between the regions of acceptance and rejection of $H_0$.
	...
	\item \textbf{p-value} - is the highest level of significance by which $H_0$ cannot be rejected.
\end{itemize}
...
\begin{multicols}{2}
	\begin{tikzpicture}[scale=0.10]
		\node at (0, 15) {Two-tailed test. $H_0$ dist.};
...
\columnbreak
	\begin{tikzpicture}[scale=0.10]
		\node at (0, 15) {One-tailed test. $H_0$ dist.};
...
\textbf{The rule is}: if p-value $< \alpha$ holds, there is evidence to reject $H_0$, thus, there is evidence to accept $H_1$.
...
\textbf{Chow's structural test} - analyze the existence of structural changes in all the model parameters, it's a particular expression of the F test, where $H_0$: No structural change (all $\delta = 0$).
...
\begin{itemize}[leftmargin=*]
	...
	\columnbreak
		...
		\end{tikzpicture}
	\end{multicols}
	\item \textbf{Formal tests} - White, Bartlett, Breusch-Pagan, etc. Commonly, $H_0$: No heteroscedasticity.
\end{itemize}
...
\begin{itemize}[leftmargin=*]
	...
	\columnbreak
		...
		\end{center}
	\end{multicols}
	\item \textbf{Formal tests} - Durbin-Watson, Breusch-Godfrey, etc. Commonly, $H_0$: No auto-correlation.
\end{itemize}
