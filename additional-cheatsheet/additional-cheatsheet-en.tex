...
% page foot
\cfoot{\href{https://github.com/marcelomijas/econometrics-cheatsheet}{\normalfont \footnotesize add0.4-en - github.com/marcelomijas/econometrics-cheatsheet - CC BY 4.0}}
...
\section*{OLS in matrix notation}
	...
	\end{itemize}
	\subsection*{Variance-covariance matrix of $\hat{\beta}$}
		\begin{center}
...
\section*{Variance-covariance matrix of $\hat{u}$}
	\begin{center}
		$\Var(\hat{u}) =$
		\scalebox{0.80}{
			$
			\begin{bmatrix}
				\Var(\hat{u}_1)            & \Cov(\hat{u}_1, \hat{u}_2) & \hdots & \Cov(\hat{u}_1, \hat{u}_n) \\
				\Cov(\hat{u}_2, \hat{u}_1) & \Var(\hat{u}_2)            & \hdots & \Cov(\hat{u}_2, \hat{u}_n) \\
				\vdots                     & \vdots                     & \ddots & \vdots                     \\
				\Cov(\hat{u}_n, \hat{u}_1) & \Cov(\hat{u}_n, \hat{u}_2) & \hdots & \Var(\hat{u}_n)
			\end{bmatrix}
			$
		}
	\end{center}
	...
	\begin{center}
		$\Var(\hat{u}) = \hat{\sigma}^2_u \cdot I_n =$
		\scalebox{0.80}{
			$
			\begin{bmatrix}
				\hat{\sigma}^2_u & 0                & \hdots & 0                \\
				0                & \hat{\sigma}^2_u & \hdots & 0                \\
				\vdots           & \vdots           & \ddots & \vdots           \\
				0                & 0                & \hdots & \hat{\sigma}^2_u
			\end{bmatrix}
			$
		}
	\end{center}
	where $I_n$ is an identity matrix of $n \times n$ elements. \\
	When there is \textcolor{cyan}{heterocedasticity} and \textcolor{magenta}{auto-correlation}, the variance-covariance matrix of $u$ has the form:
	\begin{center}
		$\Var(\hat{u}) = \hat{\sigma}^2_u \cdot \Omega =$
		\scalebox{0.80}{
			$
			\begin{bmatrix}
				\hat{\sigma}^2_{u_{\textcolor{cyan}{1}}}   & \hat{\sigma}_{u_{\textcolor{magenta}{12}}} & \hdots & \hat{\sigma}_{u_{\textcolor{magenta}{1n}}} \\
				\hat{\sigma}_{u_{\textcolor{magenta}{21}}} & \hat{\sigma}^2_{u_{\textcolor{cyan}{2}}}   & \hdots & \hat{\sigma}_{u_{\textcolor{magenta}{2n}}} \\
				\vdots                                     & \vdots                                     & \ddots & \vdots                                     \\
				\hat{\sigma}_{u_{\textcolor{magenta}{n1}}} & \hat{\sigma}_{u_{\textcolor{magenta}{n2}}} & \hdots & \hat{\sigma}^2_{u_{\textcolor{cyan}{n}}}
			\end{bmatrix}
			$
		}
	\end{center}
...
\section*{Incorrect functional form}
	\begin{itemize}[leftmargin=*]
		...
		\begin{enumerate}[leftmargin=*]
			...
			\end{center}
		\end{enumerate} % https://de.wikipedia.org/wiki/RESET-Test_nach_Ramsey
